...
__INPUT_FILE = Path(Path(__file__).parent, "task_1_1.txt")


def from_txt(input_file: Union[Path, str]) -> Tuple[List[int], List[int]]:
...
def task() -> int:
    ...
    Returns:
        The answer to task.
    """
    list_1, list_2 = from_txt(__INPUT_FILE)
    return sum(map(lambda x: abs(x[0] - x[1]), zip(sorted(list_1), sorted(list_2))))
